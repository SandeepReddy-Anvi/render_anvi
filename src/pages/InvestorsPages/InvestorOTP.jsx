...
import { verifyOTP } from "../../services/investorAuth";
import { Icon } from "@iconify/react";

export default function InvestorOTP(props) {
  const { emailFun, loginFun, sessionFun } = props;
  ...
  const handleVerifyOTP = async () => {
    try {
      ...
      const getToken = localStorage.getItem("investorToken")
      const getEmail = localStorage.getItem("investorEmail")
      // console.log(getEmail, otpString, typeof otpString, getToken);
      const res = await verifyOTP(getEmail, otpString, getToken);
      console.log(res);
      if (res.status === 200) {
        // localStorage.removeItem("investorToken");
        // localStorage.removeItem("investorEmail");

        // sessionStorage.setItem("investorSessionToken", res.data.token);
        // navigate("/investors/dashboard");

        emailFun('', false)
        loginFun('', false)
        sessionFun(res.data.token, true)
      }
      else {
        emailFun('', false)
        loginFun('', false)
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-screen font-['Wix Madefor Display']">
        <div className="bg-[#FFFFFF] p-6 rounded-[24px]">
          ...
          <button
            ...
            className="w-full mt-4 p-3 bg-[#FA293E] rounded text-white font-semibold hover:bg-[#FF8787]"
          >
            Verify
          </button>
