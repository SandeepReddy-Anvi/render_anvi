  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-6 md:gap-8 px-4 md:px-6 lg:px-0 font-['Wix Madefor Display']">
      ...
      <div
        ...
        style={{
          height: rightHeight && window.innerWidth >= 768 ? `${rightHeight}px` : "auto",
        }}
      >
        {tabs.map((tab, index) => (
          <button
            ...
            `}
          >
            <span className="mr-2 md:mr-3 text-lg sm:text-xl md:text-2xl font-bold">→</span>
            {tab.title}
          </button>
        ))}
      </div>
      ...
      <div ref={rightRef} className="flex-1 p-4 md:p-6 text-[#000000]">
        <h3 className="text-base md:text-[19px] font-medium mb-3">How it works:</h3>
        ...
        <h3 className="text-base md:text-[19px] font-medium mb-3">What it prevents:</h3>
        <ul className="list-disc ml-5 mb-6 text-sm md:text-[15px] space-y-1.5 marker:text-[8px]">
